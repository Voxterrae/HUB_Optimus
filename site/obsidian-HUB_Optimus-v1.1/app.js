(() => {
  ...
  function findModeForNode(nodeId) {
    ...
    const match = Object.entries(state.model.views).find(
  ...
  function renderComponents() {
    ...
    components.forEach((component) => {
      ...
      graphLink.addEventListener("click", () => {
        const targetMode = findModeForNode(component.id);
  ...
  function setControlsDisabled(disabled) {
    document.querySelectorAll("[data-mode], [data-filter]").forEach((control) => {
      control.disabled = disabled;
    });
    elements.sourceSearch.disabled = disabled;
  }
  ...
  function bindControls() {
    ...
    reducedMotion.addEventListener?.("change", () => {
      if (state.model) renderGraph();
    });
  }

  function renderStaticFallback() {
    if (document.getElementById("static-architecture-fallback")) return;
    const fallback = htmlElement("div", "noscript-architecture");
    fallback.id = "static-architecture-fallback";
    fallback.setAttribute("role", "status");
    fallback.append(
      htmlElement("h3", "", "Arquitectura estática"),
      htmlElement(
        "p",
        "",
        "Human review → Operator / Scenario input → validation → Simulator or Semantic Engine → local records. GitHub → Pages workflow → static site. Managed EC2 operations remain a separate, externally verified plane."
      )
    );
    elements.graphError.insertAdjacentElement("afterend", fallback);
  }
