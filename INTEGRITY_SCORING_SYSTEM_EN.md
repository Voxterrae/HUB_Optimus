...
## Integration with the simulation core (planned)

> **Note:** Automatic calculation of the Integrity Index is a **planned capability**, not a feature of the current prototype.  The existing core (`hub_optimus_simulator.py`) does not calculate this index; its current output is limited to `status`, `rounds`, `history`, and `detail`.

Once this integration is implemented, the core will calculate the Integrity Index in each round or at the end of the negotiation.  This index will be included in the reports generated by the simulator and will serve to:
