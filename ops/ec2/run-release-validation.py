...
def sealed_worker(uid: int, gid: int) -> int:
    ...
    try:
        ...
        # Python reopens /proc/self/fd after the UID transition. Give the
        # selected worker read ownership while seals keep the bytes immutable.
...
def run_validation(
    ...
    try:
        ...
        worker_descriptor = sealed_worker(uid, gid)
