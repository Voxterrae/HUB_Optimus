...
function Assert-OperationParameters {
    $delegateOperations = @(
        'exchange.test_delegated_access',
        'exchange.grant_full_access',
        'exchange.revoke_full_access'
    )
    if ($OperationId -in $delegateOperations -and
        [string]::IsNullOrWhiteSpace($Delegate)) {
        throw 'Delegate is required.'
    }
}
...
Assert-OperationParameters
Assert-MutationApproval -CurrentOperation $OperationId
# A mutation preview needs no Exchange connection or certificate access.
if ($DryRun -and $OperationId -in @('exchange.grant_full_access', 'exchange.revoke_full_access')) {
    $preview = [ordered]@{
        action = if ($OperationId -eq 'exchange.grant_full_access') {
            'Add-MailboxPermission'
        } else {
            'Remove-MailboxPermission'
        }
    }
    if ($OperationId -eq 'exchange.grant_full_access') {
        $preview['autoMapping'] = $AutoMapping
    }
    New-Result -State 'PLANNED' -Data $preview
    return
}
    ...
    switch ($OperationId) {
        'exchange.diagnose_mailbox' {
            ...
            New-Result -State 'SUCCEEDED' -Warnings $warnings.ToArray() -Data ([ordered]@{
                ...
                exchangeGuid = if ($null -ne $mailboxObject) { $mailboxObject.ExchangeGuid } else { $null }
        ...
        'exchange.grant_full_access' {
            if ($PSCmdlet.ShouldProcess($Mailbox, "Grant FullAccess to $Delegate")) {
        ...
        'exchange.revoke_full_access' {
            if ($PSCmdlet.ShouldProcess($Mailbox, "Revoke FullAccess from $Delegate")) {
