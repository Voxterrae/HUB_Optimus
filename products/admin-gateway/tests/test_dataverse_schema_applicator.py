...
def test_authorization_rejects_a_different_applicator_commit(tmp_path: Path) -> None:
    path = tmp_path / "authorization.json"
    _write_private_authorization(path, mode="APPLY_METADATA")
    with pytest.raises(app.ApplicatorError, match="applicator-commit mismatch"):
        app.load_authorization(
            path, "https://example.crm4.dynamics.com/", guid(700000), 0,
            "f" * 40, expected_mode="APPLY_METADATA",
        )
