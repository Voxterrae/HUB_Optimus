...
3. **Apply** — requires `-Apply`, an expiring private authorization file, the
   ...
   the explicitly authorized current component count (zero for the first apply). Every create request carries
...
with bounded backoff. The journal records each create after identity and membership verification. A failure between creation and verification can leave unrecorded metadata and requires private reconciliation before rollback; do not infer absence from an incomplete journal.
