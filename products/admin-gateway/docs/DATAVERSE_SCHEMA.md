...
User-owned approval or rejection record intended to be append-only through tenant authorization controls bound to one request and
...
Organization-owned audit event intended to be append-only through tenant authorization controls with an event key, correlation key,
...
alternate keys. Each created metadata ID is journaled after identity and solution-membership verification. A failure between creation and verification requires private reconciliation; the journal must not be treated as proof that no unrecorded metadata exists. Exact
