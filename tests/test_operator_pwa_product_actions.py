...
def test_operator_progress_is_immediate_and_has_no_fake_delay_plan():
    ...
    assert "hub-optimus-operator-v0-21" in sw
...
def test_operator_draft_is_source_bound_and_conservative():
    ...
    assert "review_profile=source-bound-v1" in html
...
def test_advanced_normalizer_uses_the_primary_retrieval_gate():
    ...
    assert '!$("product_confirm_source").checked' in run.group(1)
...
def test_url_retrieval_discards_stale_async_responses_and_confirmation_removal():
    ...
    assert "URL retrieval continues. Operator context remains separate" in html
    assert "advancedNormalizeButton.disabled = true" in html
    assert '$("product_confirm_source").addEventListener("change"' in html
    assert "Source confirmation was removed" in html
    assert "Source confirmation removed. The previous draft is stale and cannot be shared." in html
...
def test_url_and_operator_context_are_kept_as_distinct_inputs():
    ...
    assert '!$("product_confirm_source").checked' in body
    assert "currentRetrievedSourceText" in body
    assert "operator-provided-context-not-attributed-to-source" in html
    assert "Every supplied URL is sent alone to controlled intake" in html
    ...
    assert "Previous draft invalidated" in html
...
def test_every_case_edit_invalidates_sharing_and_share_counts_records():
    ...
    assert '...shareRecordLines("Claims", record.claims, record.claim)' in html
    assert '...shareRecordLines("Evidence", record.evidence_records, record.evidence)' in html
    assert "additional ${label.toLowerCase()} record(s) omitted from this message" in html
...
if (pasted[1] !== "Supplied URL (not fetched): https://example.com/") {
...
if (fetched[1] !== "Retrieved URL: https://final.example/") {
...
def test_operator_url_fallback_remains_actionable():
    ...
    assert "Paste the source text or relevant excerpt below, remove the unavailable URL" in html
...
def test_operator_install_assets_use_institutional_mark_and_cache_v021():
    icon = _read(ICON)
    sw = _read(SW)
