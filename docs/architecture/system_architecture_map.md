...
| Operator and static site | `site/`, `site/operator/`, and their tests | Browser-side intake and local draft preparation. Public Operator keeps URL intake local/manual; only the exact private origin may post to same-origin `/api/intake` after owner/team authentication. Operator can generate—but does not execute—a copy/paste command for a separately controlled `/analyze` runtime. The repository does not establish that the private endpoint is deployed. Operator drafts are not automatically Semantic Engine output or verified evidence. |
| Local intake and operations | `tools/mobile_ingest.py`, `ops/ec2/`, their contracts, and tests | Local intake plus a repository candidate for NGINX → Entra OIDC → bounded loopback gateway → local hub-api. It does not establish live DNS/TLS, Entra assignments, secrets, managed confidential storage, or a deployed host. |
