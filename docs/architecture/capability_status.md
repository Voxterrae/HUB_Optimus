...
| Operator browser/PWA | Prototype | `site/operator/`; `tests/test_operator_pwa_product_actions.py`; `tests/test_operator_pwa_record_integrity.py` | Browser intake has one canonical source state and prepares local drafts. Public Operator sends no URL during intake; the private-origin path is gated for owner/team OIDC. Browser output is not automatically verified evidence or Semantic Engine output. |
| Controlled single-URL intake | Partial | `docs/rfc/operator_controlled_url_intake.md`; `ops/ec2/controlled_url_intake.v1.schema.json`; `ops/ec2/operator_public_intake.v1.schema.json`; `ops/ec2/hub-api.sh`; `ops/ec2/operator-intake-gateway.py`; `tests/test_hub_api_controlled_url_intake.py`; `tests/test_ec2_operator_intake_gateway.py`; `tests/test_operator_pwa_product_actions.py` | The local contract accepts one URL and enforces documented size/network/redirect limits. The public candidate adds an authenticated, rate/concurrency-bounded envelope without changing unreviewed provenance. Neither layer crawls, verifies truth, or proves that an endpoint is deployed. |
...
| Local EC2 operational scripts | Prototype | `ops/ec2/`; `tests/test_ec2_run_identity_and_provenance.py`; `tests/test_ec2_deploy_hub_api_sync.py`; `tests/test_ec2_operator_api_proxy_config.py`; `tests/test_ec2_oauth2_proxy_config.py`; `tests/test_ec2_operator_intake_gateway.py` | Run identity, explicit deployment provenance, rollback records, API synchronization, Entra OIDC config and the loopback gateway boundary are tested in isolated fixtures. No live host, role assignment, availability, restart, DNS/TLS, security posture, or provider commitment is attested. |
