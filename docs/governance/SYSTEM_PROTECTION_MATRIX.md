...
| Human stewardship and technical review boundaries | Project ownership, repository identity, path-specific review authority, and AI assistance could be conflated. | `docs/governance/PROJECT_STEWARDSHIP.md` defines Benjamin Gerrit Hoff as creator, project owner, and primary human steward; `@Voxterrae` as repository identity; and Rodrigo / `@itteamrod` as scoped Technical CODEOWNER only for paths listed in `.github/CODEOWNERS`. | Benjamin Gerrit Hoff; scoped technical review by `@itteamrod` | active | CODEOWNERS enforcement still depends on GitHub rulesets. No authority may be inferred outside explicitly listed paths. |
...
- Keep scoped technical ownership synchronized across `.github/CODEOWNERS`, `PROJECT_STEWARDSHIP.md`, `ACKNOWLEDGEMENTS.md`, and `AI_HANDOFF.md`.
